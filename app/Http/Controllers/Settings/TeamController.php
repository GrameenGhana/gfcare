...
class TeamController extends Controller
{
    ...
    public function switchCurrentTeam(Request $request, $teamId)
    {
        ...
        $user->refreshCurrentModule();

        if (preg_match('/teams\/\d+/',$request->server('HTTP_REFERER'))) {
            $url = preg_replace('/teams\/\d+/','teams/'.$teamId,$request->server('HTTP_REFERER'));
            return redirect($url);
        } else { 
            return back();
        }
    ...
    /**
     * Switch the team module the user is currently viewing.
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  int  $moduleId
     * @return \Illuminate\Http\Response
     */
    public function switchCurrentModule(Request $request, $moduleId)
    {
        $user = $request->user();
        $module = \App\Teams\Module::find($moduleId);
        if ($module) {
            $user->switchToModule($module);
            $url =  $module->menu_slug;
            return redirect($url);
        } else {
            return back();
        }
    ...
    public function updateTeamMember(Request $request, $teamId, $userId)
    {
        ...
        if (Spark::$updateTeamMembersWith) {
            ...
            $editor = ($request->role=='editor') ? 1:0;
            
            $userToUpdate->modules()->updateExistingPivot(
                $user->current_module_id, ['role' => $request->role, 'editor'=>$editor]
            );
        }
